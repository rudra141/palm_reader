...
export function Hero() {
  return (
    <section className="relative" aria-labelledby="hero-heading">
      ...
      {/* ---------- Hero overlay (scrim + type + footer microcopy) ----------
          Wrapped in an absolute 300vh container that mirrors the Story
          sentinel; child is `sticky top-0 h-screen` so the overlay pins to
          the viewport top throughout the hero scroll and releases naturally
          when the user enters the sections below. (Pre-fix this used
          `position: fixed` and bled onto every section on the page.) */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 z-[var(--z-overlay)] h-[300vh]"
      >
        <div className="sticky top-0 h-screen w-full">
          ...
          {/* Type block */}
          <div className="absolute inset-0 flex items-end pb-[var(--space-9)] md:items-center md:pb-0">
                ...
                <div className="mt-[var(--space-6)] flex flex-wrap items-center gap-x-[var(--space-6)] gap-y-[var(--space-4)] md:mt-[var(--space-7)]">
                  <Link
                    ...
                    className={buttonStyles({
                      ...
                      size: 'lg',
                      className:
                        'group gap-[var(--space-3)] shadow-[0_2px_24px_rgba(18,16,14,0.4)]',
                    })}
                  >
          ...
          {/* ---------- Footer microcopy ----------
              Tiny disclaimer line, lives inside the sticky overlay so it
              releases with the rest of the hero on scroll. */}
          <p
            className="pointer-events-none absolute inset-x-0 bottom-[var(--space-4)] mx-auto w-full max-w-[88rem] px-[var(--space-5)] text-center text-xs font-[var(--font-body)] tracking-[var(--tracking-wide)] md:px-[var(--space-7)] md:text-left lg:px-[var(--space-8)]"
            style={{
